...
struct _CHARGERDETECTIONCONTEXT;
struct _CHIPIDEAINTERRUPTDATA;
struct _CHIPIDEA_DPC_WORKITEM_CONTEXT;
struct _CHIPIDEA_FLIGHT_RECORD_WRAPPER;
struct _CHIPIDEA_QUEUE_HEAD;
struct _CHIPIDEA_QUEUE_HEAD_PAIR;
struct _CHIPIDEA_TRANSACTION_CONTEXT;
struct _CHIPIDEA_TRANSFER_DESCRIPTOR;
...
struct _FLIGHT_RECORDER;
struct _FLIGHT_RECORDER_CONTEXT;
struct _FR_CHIPIDEA_DTD_INFO;
struct _FR_CHIPIDEA_EP_INFO;
struct _FR_CHIPIDEA_REGISTER_SET;
struct _FR_CHIPIDEA_WATCHDOG;
struct _FR_IO_QUEUE_STATUS;
struct _FR_REQUEST_INFO;
...
struct _UFXDEVICE_PRIVCONTEXT;
struct _UFXENDPOINT_INIT;
struct _UFXENDPOINT_PRIVCONTEXT;
...
struct _USBFN_INTERFACE_POWER_TEST_INTERFACE;
struct _USBFN_ON_ATTACH;
struct _USBFN_POWER_CONTEXT;
struct _USBFN_POWER_FILTER_STATE;
...
enum CLIENT_DIRECTION;
enum CLIENT_ENDPOINT_DIRECTION;
enum CLIENT_TRANSFER_DIRECTION;
enum DEVICE_REGISTRY_PROPERTY;
enum DEVICE_TEXT_TYPE;
enum DMA_COMPLETION_STATUS;
enum POWER_ACTION;
enum USBFN_BUS_IDLE_STATE;
enum UsbFnPState;
enum _CD_STATE;
...
enum _FLIGHT_RECORDER_TYPE;
enum _FLIGHT_RECORD_TYPE;
enum _FSINFOCLASS;
...
typedef struct _CHARGERDETECTIONCONTEXT {
    enum _CD_STATE State;
    struct _KEVENT StopNotification;
    struct WDFWORKITEM__ *WorkItem;
    struct WDFWAITLOCK__ *WaitLock;
    enum _USBFN_PORT_TYPE CurrentPortType;
    ULONG PendingDetachCount;
};

typedef struct _CHIPIDEAINTERRUPTDATA {
    struct {
        ULONG USBSTSRegisterValue;
        ULONG OTGSCRegisterValue;
        ULONG ENDPTSETUPSTATRegisterValue;
        ULONG ENDPTCOMPLETERegisterValue;
    } Registers;
    struct _FLIGHT_RECORDER *ISRRecorder;
    ULONG InterruptIntensity;
    UCHAR IsControllerInterruptShared;
};

typedef struct _CHIPIDEA_DPC_WORKITEM_CONTEXT {
    struct _LIST_ENTRY DpcNotifications;
    ULONG NotificationCount;
    struct WDFSPINLOCK__ *NotificationSpinLock;
    struct _KEVENT NotificationEvent;
    UCHAR ProcessingNotifications;
    struct _FLIGHT_RECORDER *WorkItemRecorder;
};

typedef struct _CHIPIDEA_FLIGHT_RECORD_WRAPPER {
    UCHAR RecordType;
    UCHAR Processor;
    USHORT Token;
    struct _KTHREAD *Thread;
    union _CHIPIDEA_FLIGHT_RECORD Record;
};

typedef struct _CHIPIDEA_QUEUE_HEAD {
    ULONG Config;
    ULONG CurrentTD;
    struct _CHIPIDEA_TRANSFER_DESCRIPTOR Overlay;
    UCHAR SetupBuf[8];
    ULONG UserData[4];
};

typedef struct _CHIPIDEA_QUEUE_HEAD_PAIR {
    struct _CHIPIDEA_QUEUE_HEAD Receive;
    struct _CHIPIDEA_QUEUE_HEAD Transmit;
};

typedef struct _CHIPIDEA_TRANSACTION_CONTEXT {
    struct _LIST_ENTRY *pTransferDescriptorListHead;
    UCHAR EndpointIndex;
    enum CLIENT_TRANSFER_DIRECTION TransactionDirection;
    ULONG TotalBytesProgrammed;
};

typedef struct _CHIPIDEA_TRANSFER_DESCRIPTOR {
    ULONG NextTD;
    ULONG Control;
    ULONG BufferPointers[5];
    ULONG UserData;
};
...
typedef struct _CONTROLLERDATA {
    struct WDFDEVICE__ *Device;
    UCHAR IsConnectedToHost;
    UCHAR InterruptsEnabled;
    UCHAR TestModeActive;
    struct _CHARGERDETECTIONCONTEXT ChargerDetection;
    PVOID pQueueHeadPhysicalBase;
    PVOID pQueueHeadVirtualBase;
    PVOID pTransferDescriptorPhysicalBase;
    PVOID pTransferDescriptorVirtualBase;
    UCHAR TestModeSelector;
    ULONG RegisterBaseAddress;
    ULONG RegisterMapLength;
    struct WDFCOMMONBUFFER__ *QueueHeadCommonBuffer;
    struct WDFCOMMONBUFFER__ *TransferDescriptorCommonBuffer;
    struct WDFDMAENABLER__ *DmaEnabler;
    struct WDFINTERRUPT__ *InterruptObject;
    ...
    struct WDFQUEUE__ *EndpointQueueArray[32];
    struct WDFSPINLOCK__ *DpcSpinLock;
    struct WDFSPINLOCK__ *EndpointPrimeSpinlock;
    struct WDFWORKITEM__ *DpcNotificationsWorkItem;
    struct WDFWORKITEM__ *EndpointRequestCompletionWorkItem;
    struct WDFLOOKASIDE__ *DpcNotificationsLookAside;
    LONG NotificationStatus;
    ULONG ProgrammedEndpoints;
    struct _FLIGHT_RECORDER *ControllerRecorder;
    struct _FLIGHT_RECORDER *WatchdogRecorder;
    struct UFXDEVICE__ *UfxDevice;
    struct _USBFN_POWER_CONTEXT PowerContext;
    enum _USBFN_DEVICE_STATE UsbState;
    enum _USBFN_PORT_TYPE UsbPort;
    UCHAR IsIdle;
    UCHAR PortSuspended;
    UCHAR NeedsExternalVBusDetection;
    UCHAR BSessionValid;
    UCHAR ResetInProgress;
    struct _LIST_ENTRY *FreeDtdListHead;
    struct WDFSPINLOCK__ *FreeDtdListSpinLock;
    UCHAR HardwareReady;
    UCHAR DisconnectPending;
    enum _USBFN_BUS_SPEED Speed;
...
typedef struct _EPQUEUEDATA {
    struct _USB_ENDPOINT_DESCRIPTOR EndpointDescriptor;
    struct _CHIPIDEA_QUEUE_HEAD_PAIR *pQueueHeadPair;
    struct WDFDMATRANSACTION__ *DmaTransaction;
    struct WDFREQUEST__ *ActiveRequest;
    UCHAR IsActiveRequestDmaTransaction;
    UCHAR DoesActiveRequestRequireZlp;
    UCHAR IsActiveRequestCancellable;
    struct WDFSPINLOCK__ *QueueDataSpinlock;
    struct WDFSPINLOCK__ *QueueTransferSpinLock;
    ULONG ProgrammedDTDs;
    struct _FLIGHT_RECORDER *EndpointRecorder;
    struct WDFTIMER__ *EPWatchdogTimer;
    struct UFXENDPOINT__ *UfxEndpoint;
};
...
typedef struct _FLIGHT_RECORDER {
    enum _FLIGHT_RECORDER_TYPE Type;
    UCHAR InUse;
    USHORT RecorderID;
    USHORT RecorderSize;
    USHORT RecordIndex;
    struct _CHIPIDEA_FLIGHT_RECORD_WRAPPER *PayLoad;
    struct _FLIGHT_RECORDER_CONTEXT RecorderContext;
};

typedef struct _FLIGHT_RECORDER_CONTEXT {
    ULONG ContextSize;
    PVOID Context;
};

typedef struct _FR_CHIPIDEA_DTD_INFO {
    struct WDFREQUEST__ *Request;
    ULONG DTDsProgrammed;
    UCHAR SGListExhausted;
    ULONG TotalBytesProgrammed;
};

typedef struct _FR_CHIPIDEA_EP_INFO {
    UCHAR EPAddress;
    UCHAR Completed;
    struct WDFREQUEST__ *Request;
    ULONG NextTD;
...
typedef struct _FR_CHIPIDEA_REGISTER_SET {
    ULONG USBSTSRegisterValue;
    ULONG OTGSCRegisterValue;
    ULONG ENDPTSETUPSTATRegisterValue;
    ULONG ENDPTCOMPLETERegisterValue;
...
typedef struct _FR_CHIPIDEA_WATCHDOG {
    UCHAR EpAddress;
    ULONG ENDPTCOMPLETERegisterValue;
    ULONG USBSTSRegisterValue;
    ULONG QHControl;
...
typedef struct _FR_IO_QUEUE_STATUS {
    UCHAR EpAddress;
    enum _WDF_IO_QUEUE_STATE State;
    ULONG QueueRequests;
    ULONG DriverRequests;
};

typedef struct _FR_REQUEST_INFO {
    struct WDFREQUEST__ *Request;
    LONG Status;
    ULONG64 BytesProgrammed;
...
typedef struct _PEPStateResultType {
    LONG PEPStatus;
    LONG Status;
    ULONG Index;
};
...
typedef struct _UFXDEVICE_PRIVCONTEXT {
    ULONG Sig;
    struct WDFDEVICE__ *WdfDevice;
    struct UFXDEVICE__ *UfxDevice;
...
typedef struct _UFXENDPOINT_PRIVCONTEXT {
    ULONG Sig;
    struct WDFDEVICE__ *WdfDevice;
    struct UFXDEVICE__ *UfxDevice;
    struct _USB_ENDPOINT_DESCRIPTOR EndpointDescriptor;
...
typedef struct _USBFN_INTERFACE_POWER_TEST_INTERFACE {
    struct _INTERFACE InterfaceHeader;
    PUCHAR UsbFnBusEnterIdleState;
};
...
typedef struct _USBFN_POWER_CONTEXT {
    UCHAR PowerManagementInitialized;
    struct POHANDLE__ *PoFxHandle;
    struct _USBFN_POWER_FILTER_STATE CurrentPowerFilterState;
    struct WDFREQUEST__ *PowerFilterRequest;
    struct _KEVENT PStateTransitionEvent;
    struct _PEPStateResultType PStateTransitionResult;
    enum UsbFnPState PState;
    UCHAR ArmedForWake;
    struct _USBFN_INTERFACE_POWER_TEST_INTERFACE PowerTestInterface;
    PVOID PowerTestPnPNotificationContext;
    struct WDFDEVICE__ *PowerTestClassDevice;
    UCHAR InP0State;
};

typedef struct _USBFN_POWER_FILTER_STATE {
    enum _USBFN_DEVICE_STATE DeviceState;
    ULONG PState;
    ULONG Reserved;
};
...
typedef union _CHIPIDEA_FLIGHT_RECORD {
    UCHAR RawData[16];
    struct _FR_CHIPIDEA_DTD_INFO DTDInfo;
    struct _FR_REQUEST_INFO RequestInfo;
    struct _FR_CHIPIDEA_REGISTER_SET Registers;
    struct _FR_CHIPIDEA_EP_INFO EpInfo;
    struct _FR_CHIPIDEA_WATCHDOG WatchdogInfo;
    struct _FR_IO_QUEUE_STATUS QueueStatus;
};
...
typedef enum CLIENT_DIRECTION {
    ClientDirectionOut = 0,
    ClientDirectionIn = 1
};

typedef enum CLIENT_ENDPOINT_DIRECTION {
    ClientEndpointDirectionOut = 0,
    ClientEndpointDirectionIn = 1,
    ClientEndpointDirectionRx = 0,
    ClientEndpointDirectionTx = 1,
    ClientEndpointDirectionBidirectional = 2,
    ClientEndpointDirectionMaximum = 3
};

typedef enum CLIENT_TRANSFER_DIRECTION {
    ClientTransferDirectionOut = 0,
    ClientTransferDirectionIn = 1,
    ClientTransferDirectionTx = 1,
    ClientTransferDirectionRx = 0
};
...
typedef enum USBFN_BUS_IDLE_STATE {
    UsbfnBusIdleStateInvalid = 0,
    UsbfnBusIdleStateIdle = 1,
    UsbfnBusIdleStateActive = 2
};

typedef enum UsbFnPState {
    P0_USB_ATTACHED_SDP = 0,
    P1_USB_ATTACHED_CDP = 1,
    P2_USB_ATTACHED_DCP_OR_PDCP = 2,
    P3_USB_ATTACHED_IDCP = 3,
    P4_USB_SUSPENDED = 4,
    P5_USB_DETACHED = 5,
    PSTATE_MAXIMUM = 6
};

typedef enum _CD_STATE {
    CD_INVALID_MIN = 0,
    CD_DETACHED = 1,
    CD_DETECTION_STARTED = 2,
    CD_DCD_STARTING = 3,
    CD_DCD_WAITING = 4,
    CD_DCD_DONE = 5,
    CD_PPD_STARTING = 6,
...
typedef enum _FLIGHT_RECORDER_TYPE {
    FlightRecorderEndpoint = 0,
    FlightRecorderISR = 1,
    FlightRecorderController = 2,
    FlightRecorderWorkItem = 3,
    FlightRecorderWatchdog = 4,
    FlighRecorderInstanceMaximum = 5
};

typedef enum _FLIGHT_RECORD_TYPE {
    FlightRecordEmpty = 0,
    FlightRecordRaw = 1,
    FlightRecordDTDInfo = 2,
    FlightRecordRequestInfo = 3,
    FlightRecordChipideaRegistersSet = 4,
    FlightRecordEpInfo = 5,
    FlightRecordWatchdog = 6,
    FlightRecordIOQueueStatus = 7,
    FlightRecordMaximum = 8
};
...
EndpointInformation;
EnterLPM;
EpWatchdogTimerStart;
EpWatchdogTimerStop;
EpWatchdogWorkItemBegin;
EpWatchdogWorkItemEnd;
EpWatchdogWorkItemRegs;
ExitLPM;
GUID_ACPI_INTERFACE_STANDARD2;
GUID_DEVINTERFACE_USBFNTEST_POWER_TEST;
GUID_PDC_IDLE_RESILIENCY_ENGAGED;
GUID_POWER_CHANGE_P_STATE;
GUID_SPM_LOW_POWER_CS;
GUID_UFX_INTERFACE_PROPRIETARY_CHARGER;
GUID_USBFN_INTERFACE_ATTACH;
GUID_USBFN_INTERFACE_POWER_TEST;
LPMEntryFailed;
LPMExitFailed;
Microsoft_WindowsPhone_UfxChipideaEnableBits;
Microsoft_WindowsPhone_UfxChipideaKeywords;
Microsoft_WindowsPhone_UfxChipideaLevels;
PStateTransitionFailed;
PortDetection;
Register;
RegisterInformation;
TotalStall;
...
WDF_CHIPIDEAINTERRUPTDATA_TYPE_INFO;
WDF_CHIPIDEA_DPC_WORKITEM_CONTEXT_TYPE_INFO;
WDF_CHIPIDEA_TRANSACTION_CONTEXT_TYPE_INFO;
WDF_COMMAND_QUEUE_CONTEXT_TYPE_INFO;
WDF_CONTROLLERDATA_TYPE_INFO;
WDF_EPQUEUEDATA_TYPE_INFO;
WDF_ETW_ERROR_RECORD_CONTEXT_TYPE_INFO;
WDF_PROPRIETARY_CHARGER_CONTEXT_TYPE_INFO;
WDF_UFXDEVICE_PRIVCONTEXT_TYPE_INFO;
WDF_UFXENDPOINT_PRIVCONTEXT_TYPE_INFO;
WNF_PO_SCENARIO_CHANGE;
WPPTraceSuite;
WPP_11798fe700973537515f7a54f809b419_Traceguids;
WPP_222728038bbd35571dba235fa052df0a_Traceguids;
WPP_48b4b617bf7a3b30e2993188ab9c6c44_Traceguids;
WPP_6076512044383a34a741d58cf4995ffa_Traceguids;
WPP_6a84a48e3545382bd8dba998c52257f4_Traceguids;
WPP_7047639b513b30b72ab41dae27491a76_Traceguids;
WPP_7524289d4dc332f11498b05955e281eb_Traceguids;
WPP_78b7a2591549327af030eee0ce5cf7a2_Traceguids;
WPP_GLOBAL_Control;
WPP_MAIN_CB;
WPP_ThisDir_CTLGUID_UsbFnChipidea;
WPP_c21f229f8a6e308aa87ebfad91f8ce80_Traceguids;
WPP_c4a43df962a63f7f852eb8f71c87cb2c_Traceguids;
WPP_d239bab4e22c39294b50035dc4ee672f_Traceguids;
WPP_ecd624927e10353748707cda9034e68e_Traceguids;
WPP_f79fc01b3a6f3246f902cecae6817b94_Traceguids;
WdfBindInfo;
WdfDriverGlobals;
WdfFunctions_01015;
...
`ChipIdeaFlightRecorderGetRecord'::`2'::FlightRecordNumber;
gChipIdeaRecorders;
gCsNotifyCallback;
gEtwContext;
...
ChargerDetectionReset();
ChargerDetectionStart();
ChargerDetectionStop();
ChargerDetectionWorkItem();
ChipIdeaAcpiInvokeMethod();
ChipIdeaCancelActiveRequest();
ChipIdeaEnQueueDpcNotification();
ChipIdeaEndPointWatchdog();
ChipIdeaEndpointComplete();
ChipIdeaEndpointCompleteCreation();
ChipIdeaEndpointConfigure();
ChipIdeaEndpointDisable();
ChipIdeaEndpointEnable();
ChipIdeaEndpointGetQueueProperties();
ChipIdeaEndpointIoStop();
ChipIdeaEndpointQueueCleanup();
ChipIdeaEndpointResetTransferState();
ChipIdeaEndpointSetStallState();
ChipIdeaEndpointTransactionCleanup();
ChipIdeaEndpointTransactionObjectInitialize();
ChipIdeaEvtProgramDmaOperation();
ChipIdeaEvtTransferRequestCancel();
ChipIdeaFreeTransferDtdListInitialize();
ChipIdeaFreeTransferDtdRelease();
ChipIdeaFreeTransferDtdRetrieve();
ChipIdeaGetFlightRecorder();
ChipIdeaInitializeRecorderContext();
ChipIdeaInterruptAddNotification();
ChipIdeaInterruptCompletePendingRequest();
ChipIdeaInterruptHandleEndpointCompletion();
ChipIdeaInterruptHandleReset();
ChipIdeaInterruptHandleResetComplete();
ChipIdeaInterruptHandleSetupPacket();
ChipIdeaInterruptProcessNotification();
ChipIdeaIsUrsEnumerated();
ChipIdeaMarkActiveRequestCancellable();
ChipIdeaReleaseFlightRecorder();
ChipIdeaSendSynchronousIoctlToPdo();
ChipIdeaTransactionBegin();
ChipIdeaTransferAddDtd();
ChipIdeaTransferAddZlpDtd();
ChipIdeaTransferCancel();
ChipIdeaTransferGetBytesTransferred();
ChipIdeaTransferIsTransactionComplete();
ChipIdeaTransferIssue();
ChipIdeaTransferPrime();
ChipIdeaTransferProgramDtdList();
ChipIdeaTransferRetireAllDtds();
ChipIdeaUnmarkCancellableAndCancelActiveRequest();
ChipideaAbortGetAttachActionWorker();
ChipideaAcpiEvaluateDsmMethod();
ChipideaAcpiGetPortType();
ChipideaAcpiInitializeAttachAndPortDetect();
ChipideaAcpiNotifyCallback();
ChipideaAcpiRegisterForNotification();
ChipideaAcpiSetDeviceState();
ChipideaAcpiUnregisterForNotification();
ChipideaAttachAbortOperation();
ChipideaAttachInterfaceEnqueueGetAttachAction();
ChipideaAttachInterfaceInitialize();
ChipideaAttachInterfaceSetDeviceState();
ChipideaAttachIsr();
ChipideaCompleteEndpointCreation();
ChipideaControllerWakeIsr();
ChipideaCreateDpcWorkItems();
ChipideaCreateInterruptObject();
ChipideaCsNotifyCallback();
ChipideaDetachIsr();
ChipideaDpc();
ChipideaDpcNotificationsWorkItem();
ChipideaEndpointDeviceIoControl();
ChipideaEtwLogCsInfo();
ChipideaEtwRundown();
ChipideaEvtDeviceAdd();
ChipideaEvtDeviceArmWakeFromS0();
ChipideaEvtDeviceD0Entry();
ChipideaEvtDeviceD0Exit();
ChipideaEvtDeviceInternalIoControl();
ChipideaEvtDeviceIoControl();
ChipideaEvtDevicePrepareHardware();
ChipideaEvtDeviceReleaseHardware();
ChipideaEvtDriverUnload();
ChipideaGetAttachActionWorker();
ChipideaGetEndpointStallState();
ChipideaInitializeDevice();
ChipideaInterruptsEnable();
ChipideaIsr();
ChipideaPowerAssignPowerFrameworkSettings();
ChipideaPowerAssignS0IdleSettings();
ChipideaPowerCompletePStateTransition();
ChipideaPowerControlCallback();
ChipideaPowerEvtNotifyTestInterfaceChange();
ChipideaPowerFilterExitLPM();
ChipideaPowerFilterSetState();
ChipideaPowerGetPStateFromDevice();
ChipideaPowerInitialize();
ChipideaPowerPepSetPState();
ChipideaPowerUninitialize();
ChipideaPowerWdmEvtDeviceWdmPostPoFxRegisterDevice();
ChipideaPowerWdmEvtDeviceWdmPrePoFxRegisterDevice();
ChipideaReportInterruptsActive();
ChipideaReportInterruptsInactive();
ChipideaSetEndpointInformation();
ChipideaSetEndpointStallState();
ChipideaSetTestMode();
ChipideaUnInitializeDevice();
ChipideaVirtualAttach();
ChipideaVirtualDetach();
CsNotifyWnfCallback();
DeInitChipIdeaMemory();
DriverEntry();
...
InitChipIdeaMemory();
InitializeChargerDetectionContext();
...
McTemplateK0xq();
McTemplateK0xqz();
PerformChargerDetection();
PerformDataContactDetection();
PerformPrimaryPortDetection();
PerformSecondaryPortDetection();
...
ReadULPIReg();
ReadULPIRegAndCheckContinuation();
RegisterForCsNotifications();
UfxDeviceStopOrResumeIdle();
UfxDevice_CreateDevice();
...
WppTraceCallback();
WriteULPIReg();
WriteULPIRegAndCheckContinuation();
_GSHandlerCheck();
