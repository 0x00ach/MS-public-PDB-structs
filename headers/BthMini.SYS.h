...
enum POWER_ACTION;
enum _BTHX_HCI_PACKET_TYPE;
enum _BTHX_SCO_SUPPORT;
...
typedef struct _BTHX_CAPABILITIES {
    ...
    enum _BTHX_SCO_SUPPORT ScoSupport;
    ULONG MaxScoChannels;
    UCHAR IsDeviceIdleCapable;
    UCHAR IsDeviceWakeCapable;
};
...
typedef struct _DEVICE_EXTENSION {
    struct _BTDEVICE *pBtDevice;
    struct _DEVICE_OBJECT *pTopOfStack;
    struct _IO_REMOVE_LOCK RemoveLock;
    ULONG PendingIrpCount;
    struct _KEVENT IrpsCancelledEvent;
    enum _DEVSTATE DeviceState;
    ULONG64 SpinLockDeviceState;
    struct _LIST_ENTRY DevExtListEntry;
    struct _WRITE_DATA_CONTEXT *pWriteCommandContext;
    struct _KEVENT CmdContextAvailableEvent;
    struct _LOCKED_LIST PendingWriteDataQueue;
    struct _READ_DATA_CONTEXT *pReadContexts;
    struct _BTHX_CAPABILITIES Capabilities;
    struct _BTHX_VERSION LTDBthXDDIVersion;
    ULONG InvalidReadStatusCount;
};
...
typedef struct _READ_DATA_CONTEXT {
    ...
    ULONG ReadInterlock;
    struct _IRP *pIrp;
    enum _BTHX_HCI_PACKET_TYPE RequestType;
    ULONG RequestDataLen;
    struct _BTHX_HCI_READ_WRITE_CONTEXT *pReadDataBuffer;
    UCHAR Cancelled;
};
...
typedef struct _WRITE_DATA_CONTEXT {
    ...
    struct _LIST_ENTRY ListEntry;
    ULONG BytesTransferred;
    struct _BTHX_HCI_READ_WRITE_CONTEXT *pWriteDataBuffer;
...
BTHMINI_EVENT_PROVIDER;
BTHMINI_EVENT_PROVIDER_Context;
BthLib_RefObjGlobals;
E_InvalidDataPacket;
E_InvalidEventPacket;
E_LFDVersionNotSupported;
Microsoft_Windows_Bluetooth_BthminiEnableBits;
Microsoft_Windows_Bluetooth_BthminiKeywords;
Microsoft_Windows_Bluetooth_BthminiLevels;
WPPTraceSuite;
WPP_2174f690687e383c9b017c524b507d4f_Traceguids;
WPP_GLOBAL_Control;
WPP_MAIN_CB;
WPP_ThisDir_CTLGUID_bthmini;
...
_imp_BTHPORT_FatalHardwareError;
...
BthMini_CancelPendingIRPs();
BthMini_CommandBipCancelRoutine();
BthMini_CreateDeviceExtension();
BthMini_DestroyDeviceExtension();
BthMini_DestroyWriteContext();
BthMini_GetCapabilities();
BthMini_GetDeviceState();
BthMini_HciWrite();
BthMini_InitDeviceExtension();
BthMini_OnReadComplete();
BthMini_PnpAddDevice();
BthMini_PnpRemoveDevice();
BthMini_PnpStartDevice();
BthMini_PnpStopDevice();
BthMini_Power();
BthMini_PushReadBipComplete();
BthMini_QueryPowerDeviceEnable();
BthMini_QueryPowerDeviceWakeEnable();
BthMini_QueueReadIRP();
BthMini_ScoAddChannel();
BthMini_ScoControlChannel();
BthMini_ScoRemoveChannel();
BthMini_ScoValidateChannel();
BthMini_SetDeviceState();
BthMini_StartReadPump();
BthMini_SubmitIrpAsync();
BthMini_SubmitIrpSync();
BthMini_Unload();
BthMini_WriteHCIData();
BthMini_WriteHCIDataComplete();
BthMini_WriteHciCommand();
BthMini_WriteHciCommandComplete();
BthOpenKey();
BthQueryKeyValueUlong();
BthVerif_BugCheckIfVerifierOn();
BthVerif_IsBthVerifDisabled();
DecrementPendingIrpRefCount();
DriverEntry();
GsDriverEntry();
